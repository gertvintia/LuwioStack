...
`<GoogleAnalyticsProvider>` + `useAnalytics` (`track` / `pageview` / `set` / raw `gtag`), loading
the script once per Measurement ID and queuing events fired before it's ready. Tracking is
consent-gated by the provider's `enabled` flag (void no-ops when off), and Google Consent Mode v2
is supported via the optional `consent` prop (default + update signals) and `updateConsent()`.
