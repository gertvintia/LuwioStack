...
`<GoogleAnalytics>`, loading the script once per Measurement ID and queuing events fired before
it's ready. `useAnalytics()` mirrors `useGoogleMaps` — load state under an `api` object plus the
actions (`track` / `pageview` / `identify` / `setUserProperties` / `updateConsent` / raw `gtag`) —
with a `useSuspenseAnalytics` variant that suspends until ready. A single `consent` prop controls
it: a boolean gate (void no-ops + no script load when `false`), or an object of per-category
booleans that map to Google Consent Mode v2 signals — applied as a `default` before config and
re-sent as an `update` when they change (also via `updateConsent()`).
