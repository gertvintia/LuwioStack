...
/**
 * What `useAnalytics` returns: the gtag.js load state plus tracking actions. Every action is a
 * void no-op while `enabled` is `false` (i.e. `consent` is `false`).
 */
export interface GoogleAnalyticsApi {
  ...
  /** `true` once gtag.js has loaded (`status === 'success'`). */
  isReady: boolean
